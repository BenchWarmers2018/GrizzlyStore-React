...
export class GoogleLogin extends Component {
    ...
    render() {
        ...
        return (
            <div>
                {
                    // testing if data is retrieved when Google Logged-in

                    // this.state.isSignedIn ? (
                    //         <span className="signed-in-google">
                    //             <div>You are now logged in as <strong>{this.state.current.displayName}</strong></div>
                    //             <button type="button" className="google-button" onClick={() => firebase.auth().signOut()}>
